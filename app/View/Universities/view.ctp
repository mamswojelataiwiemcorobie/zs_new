...
<div class="info-box shadow-large bottom0">
	...
	<?php 
		echo $this->Form->create('University', array('action' => 'view', 'class' => 'form-inline', 'role' => 'form'));
...
<div id="uniwersytet">
			...
			<div class="panel panel-default">
				<div class="panel-body">
					...
					<div class="col-xs-6 col-sm-6 col-md-4">
						<div class="btn-group-vertical linki">
							<?php if(!empty($university['UniversitiesParameter']['www'])): ?>
								<div type="button" class="btn btn-primary">
										<a href="<?php $university['UniversitiesParameter']['www'] ?>">www</a>
								</div>
							<?php endif;?>
							<div type="button" class="btn btn-default btn-lg">
								<a href="<?php $slug = Inflector::slug($university['University']['nazwa'],'-');
									echo 'http://www.zostanstudentem.pl/uczelnia/'.$slug.'-'.$university['University']['id'].".html";?>" alt="Zobacz więcej" target="_blank"><span class="glyphicon glyphicon-th"></span> Zobacz więcej</a>
							</div>
						</div>
					</div>
				</div>
		...
		<div class="col-xs-6 col-sm-6 col-md-6">
			<div class="panel panel-default">
				<div class="panel-body">
					...
					<div class="col-xs-6 col-sm-6 col-md-4">
						<div class="btn-group-vertical linki">
							<?php if(!empty($university2['UniversitiesParameter']['www'])): ?>
								<div type="button" class="btn btn-primary">
										<a href="<?php $university2['UniversitiesParameter']['www'] ?>">www</a>
								</div>
							<?php endif;?>
							<div type="button" class="btn btn-default btn-lg">
								<a href="<?php $slug = Inflector::slug($university2['University']['nazwa'],'-');
									echo 'http://www.zostanstudentem.pl/uczelnia/'.$slug.'-'.$university2['University']['id'].".html";?>" alt="Zobacz więcej" target="_blank"><span class="glyphicon glyphicon-th"></span> Zobacz więcej</a>
							</div>
						</div>
					</div>
				</div>
	...
	<div class="row">
		<div class="col-xs-6 col-sm-6 col-md-6 pie">	
			<?php if (!empty($university['University']['il_st_bezpl']) || !empty($university['University']['il_st_pl'])) :?>
			<div class="vuzz-pricing popular">
				<h2>Ilość studentów stacjonarnych i niestacjonarnych:</h2>
				<div id="chart_publiczne1" class="sto" ></div>		
			</div>
			<?php endif;?>
		</div>
		<div class="col-xs-6 col-sm-6 col-md-6 pie">
			<?php if (!empty($university2['University']['il_st_bezpl']) || !empty($university2['University']['il_st_pl'])) :?>
			<div class="vuzz-pricing popular">
				<h2>Ilość studentów stacjonarnych i niestacjonarnych:</h2>
				<div id="chart_publiczne2" class="sto"></div>		
			</div>
			<?php endif;?>
		</div>
	</div>
												...
												foreach ($university['Exchange'] as $e){
													if($e['miasto']==$mi){
														...
														if(!in_array($slug1, $doubler_uni)){
															$slug1=Inflector::slug($e['nazwa_uczelni'],'-');
															$doubler_uni[]= $slug1;
															echo '<li>'. $this->Html->link( $e['nazwa_uczelni'], '/erazmus/'.$slug1.'-'.$e['id']).'</li>';
		...
		<div class="col-xs-6 col-sm-6 col-md-6">
			...
			<?php if (!empty($university2['Exchange'])):?>
				...
				<ul>
					...
					<?php $doubler_kraj2[]="";
						$slug2 = Inflector::slug($ex2['kraj'],'-');
							if(!in_array($slug2, $doubler_kraj2)): 
					?>
						<div >
							<div class="accordion-toggle collapsed" data-toggle="collapse" data-parent="#accordion" href="#<?php echo  $ex2['kraj'];?>2"  onclick=" if (this.className=='active'){ this.className=''; } else {this.className='active'}">
								<h4 >
								<a class="accordion-toggle" data-toggle="collapse" data-parent="#accordion" href="#<?php echo  $ex2['kraj'];?>2" >
									<?php 	//echo $slug = Inflector::slug($ex['kraj'],'-');
											$doubler_kraj2[]= $slug2;
											echo $ex2['kraj'];
											$pa= $ex2['kraj']?>
							...
							<div  id="<?php echo $ex2['kraj']; unset( $ex2['kraj']);?>2" class="panel-collapse collapse">
								<div class="panel-body">
								<ul>
									<?php
 ...
 <script type="text/javascript">
	...
	<?php if ( $university['City']['id'] == $university2['City']['id'] ) :?>
		...
		var ceny_options = {
			vAxis: {title: '[PLN]', titleTextStyle: {color: 'red'}, logScale: true, minValue: 0},
			colors: ['#3276b0'],
		};
	<?php else :?>
		var ceny_data = google.visualization.arrayToDataTable([
