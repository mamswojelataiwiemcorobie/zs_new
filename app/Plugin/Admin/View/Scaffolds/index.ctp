                ...
                <?php foreach ($associations as $_type => $_data): ?>
                    <?php foreach ($_data as $_alias => $_details): ?>
                        <?php if ($_details['controller'] != $this->name && !in_array($_details['controller'], $done)): ?>
                            <li>1<?php echo $this->Html->link(__d('cake', 'List %s', Inflector::humanize($_details['controller'])), array('plugin' => 'admin', 'controller' => $_details['controller'], 'action' => 'index')); ?></li>
                            <li>2<?php echo $this->Html->link(__d('cake', 'New %s', Inflector::humanize(Inflector::underscore($_alias))), array('plugin' => 'admin', 'controller' => $_details['controller'], 'action' => 'add')); ?></li>
                ...
                <?php foreach (${$pluralVar} as ${$singularVar}): ?>
                    <tr>
                        <?php foreach ($toDisplayField as $_field): ?>
                            ...
                            <?php endif; ?>
                        <?php endforeach; ?>
                        <td><?php echo $this->Html->link(__d('cake', 'View'), array('plugin' => 'admin', 'action' => 'view', ${$singularVar}[$modelClass][$primaryKey]), array('class' => 'btn btn-info')); 
                            //echo ${$singularVar}[$modelClass][$primaryKey];//+
                        ?>xx</td>
